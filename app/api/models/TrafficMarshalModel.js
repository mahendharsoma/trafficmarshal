...
async function ensureSchema() {
  ...
  const ddl = [
    ...
    `CREATE TABLE IF NOT EXISTS senior_families (
      ...
      maps_url LONGTEXT DEFAULT NULL,
      lat DECIMAL(10,7) DEFAULT NULL,
      lng DECIMAL(11,7) DEFAULT NULL,
  ...
  try { await p.query("ALTER TABLE senior_families ADD COLUMN maps_url LONGTEXT DEFAULT NULL"); } catch {}
...
// ====== MAIN HANDLER ======
export async function handleTrafficMarshalModel(request) {
    ...
    if (method === 'POST' && parts.length === 1) {
      ...
      await p.query(
        `INSERT INTO senior_families
         ...
         maps_url,lat,lng,risk_category,created_by)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
  ...
  if (parts[0] === 'admin') {
    ...
    if (parts[1] === 'stats' && method === 'GET') {
      // Marshal counts
      const [[{ total }]] = await p.query("SELECT COUNT(*) AS total FROM marshals WHERE role='marshal'");
      const [[{ active }]] = await p.query("SELECT COUNT(*) AS active FROM marshals WHERE status='active' AND role='marshal'");
      const [[{ online }]] = await p.query("SELECT COUNT(*) AS online FROM marshals WHERE status='online' AND role='marshal'");
      const [[{ offline }]] = await p.query("SELECT COUNT(*) AS offline FROM marshals WHERE status='offline' AND role='marshal'");
      
      // Volunteer counts
      const [[{ volunteerTotal }]] = await p.query("SELECT COUNT(*) AS volunteerTotal FROM marshals WHERE role='volunteer'");
      const [[{ volunteerActive }]] = await p.query("SELECT COUNT(*) AS volunteerActive FROM marshals WHERE status='active' AND role='volunteer'");
      const [[{ volunteerOnline }]] = await p.query("SELECT COUNT(*) AS volunteerOnline FROM marshals WHERE status='online' AND role='volunteer'");
      const [[{ volunteerOffline }]] = await p.query("SELECT COUNT(*) AS volunteerOffline FROM marshals WHERE status='offline' AND role='volunteer'");
      ...
      return ok({ 
        stats: { 
          ...
          volunteers: {
            ...
            active: volunteerActive,
            online: volunteerOnline,
            offline: volunteerOffline
          },
          // System stats
          incidents, 
          open, 
